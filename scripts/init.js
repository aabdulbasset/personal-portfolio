...
async function createSpace() {
    try {
        let spaceCreated = await client.createSpace({ name: "next portfolio", defaultLocale: "en-US" })
...
async function createContentTypes(envID, spaceID) {
    ...
    let profile_info = await env.createContentType({
        name: "portfolio_info",
        fields: [
            {
                id: "name",
    ...
    let technologies = await env.createContentType({
        name: "technologies",
        fields: [{
            id: "name",
            name: "name",
    ...
    let projects = await env.createContentType({
        name: "Project",
        fields: [
            {
                id: "title",
...
async function createOrFindEnvironment(spaceID) {
    ...
    if (envID) {
        ...
        try {
            let space = await client.getSpace(spaceID)
            let envID = await space.createEnvironmentWithId("master", { name: "master" })
